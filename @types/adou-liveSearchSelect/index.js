...
const LiveSearchSelect = (props) => {
    ...
    const [value, setValue] = (0, react_1.useState)(""); // 给个 || ""就会让 input为受控状态，不能让它默认是 defaultValue，有可能不存在。。
    ...
    const prevSelectedValueRef = (0, react_1.useRef)(""); // 不用 defaultValue的原因同上
    const searchValueRef = (0, react_1.useRef)(defaultValue || "");
    const [showOptions, setShowOptions] = (0, react_1.useState)(false);
    const [filterdOptions, setFilterdOptions] = (0, react_1.useState)(options);
    ...
    return react_2.default.createElement("div", { className: cls },
        react_2.default.createElement("input", { value: value, onChange: (e) => handleInputChange(e), onClick: handleInputClick, ref: inputRef, disabled: disabled, type: "text", className: "form-control", "aria-label": "Username", "aria-describedby": "basic-addon1" }),
        showOptions && react_2.default.createElement("div", { className: "option-wrapper" }, filterdOptions.length ? filterdOptions.map((option, index) => {
            // --巧妙
            return react_2.default.createElement("div", { ref: optionItemRefs[index], key: index, onClick: (e) => handleSelect(e, option), className: "option-item" }, option.label);
        }) : react_2.default.createElement("div", { className: "option-item" }, "No match content")));
};
