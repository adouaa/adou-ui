...
const FormItem = ({
  ...
  const validateField = (
    ...
    if (children && Array.isArray(children) && children.length > 1) {
      for (const rule of rules) {
        if (rule.required) {
          for (const child of children) {
            ...
            const childValue =
              childName === fieldName ? value : data?.[childName!];
            if (!childValue) {
              isValid = false;
              setIsError(true);
      ...
      const validateValue = !isForm
        ? value
        : value || data[fieldName!] || data[name!];
      for (const rule of rules) {
  ...
  return (
    <div
      ...
      style={{
        ...
        ...(layout === "inline" &&
          !formItemWrapperWidth &&
          !formItemWrapperMaxWidth && { flex: 1 }),
      }}
    >
      {/*  ${
      ...
      <div
        className={`adou-form-item-content ${judgeFormItemContentCls()} ${
          isError ? "border-danger" : ""
        } ${
          isError && layout !== "horizontal-top" && !isCheckbox
        ...
        {label && (
          <div
            className={`adou-form-item-label-box text-end pe-3 position-relative ${
              rules && rules.length && rules.some((item: any) => item.required)
                ? "required"
                : ""
            } ${
              layout === "vertical"
