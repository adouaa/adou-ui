...
const Select = React.forwardRef((props: SelectProps, ref) => {
  const {
    ...
    noWrap = true,
    shouldFocus = false,
    activeColor = { font: '#fff', bgc: 'var(--bs-primary)' },
    returnType = 'obj',
    ...
    if (isShow) {
      ...
      if (multiple) {
        ...
        if (mode === 'common') {
          ...
          handleValidate(
            selectValue?.[valueKey] || selectValue?.[labelKey] || inputRef.current.value,
          )
        }
      }
      setClosing(true)
  ...
  const judgeColor = (item: any, type: 'font' | 'bgc') => {
    if (multiple || mode === 'tags') {
      return selectValueList?.find((option: any) => option[valueKey] == item[valueKey])
        ? activeColor?.[type]
        : type === 'font'
        ? '#000'
        : ''
  ...
  return (
    <div
      ...
      style={{
        ...wrapperStyle,
        ...(wrapperWidth ? { width: wrapperWidth } : { width: '100%', flex: 1 }),
      }}
    >
      ...
      <div className={`adou-select-form-content`}>
        ...
        >
            ...
            ellipsis ? ( // 如果 ellipsis 为 true，则需要超出省略
              (!isInputFocusing || multiple) && !showSearch && mode !== 'liveSearch' ? (
                <div
                  ...
                  className={`adou-select-value ps-2 ${contentWrap ? 'ellipsis-1' : ''}`} // ellipsis-1 加上这个，选择框会自动变大或者变小
                  style={{
              ...
              <div
                className={`adou-select-value ps-2 ${contentWrap ? 'ellipsis-1' : ''}`}
          ...
          {clearable &&
          isEnter &&
          (multiple ? selectValueList.length : selectValue?.[valueKey || labelKey]) ? (
          ...
          {showIcon ? (
            ...
            >
              <i
                ...
                className={`adou-select-icon ${isShow ? 'rotate-up' : 'rotate-down'}`}
              >
                <svg
                  className={`${isShow ? 'rotate-up' : 'rotate-down'}`}
                  width="10px"
                  height="10px"
                  viewBox="0 -4.5 20 20"
                >
