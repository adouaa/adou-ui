...
const Tabs = (props: TabsProps) => {
          ...
          <>
              {tabStyle === 'common' ? (
                  <div className="header-wrapper">
                      <div className="tabs-header mb-2">
                              ...
                              return (
                                  <div key={index}>
                                      <div className={`tabs-header-item-box ${index === 0 && 'first'}`}>
                                          <div onClick={() => handleLabelClickFn(index, child)} className={`tabs-header-item  ${currentIndex === index && 'active'}`}>
                                              {child?.props?.label}
                                          </div>
                                      </div>
                                  </div>
                      ...
                      {showExtraContent && <div className="extra-content">{content.current}</div>}
                  </div>
              ) : (
                  <div className="header-wrapper">
                              ...
                              return (
                                  <li key={index} className="nav-item d-flex" onClick={() => handleLabelClickFn(index, child)}>
                                      <a
                                          ...
                                          className={`${index === currentIndex ? 'active' : ''} ${headerItemExternalCls} nav-link`}
                                          aria-current="page"
                                      >
                      ...
                      {showExtraContent && <div className={`extra-content ${extraContentCls}`}>{content.current}</div>}
                  </div>
              )}
          </>
      );
  ...
  const renderContent = () => {
      ...
      return (
          <div className="tab-content" style={{ ...(contentHeight ? { height: contentHeight } : {}) }}>
              {renderChildren}
          </div>
      );
  };
