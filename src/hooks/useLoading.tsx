...
const Loading = ({ maskStyle, loadingStyle }: LoadingProps) => {
    ...
    const enhancedMaskStyle = Object.keys(maskStyle || {}).length > 0 ? maskStyle : { backgroundColor: 'rgba(255, 255, 255, 0.8)' };
    const enhancedLoadingStyle = Object.keys(loadingStyle || {}).length > 0 ? loadingStyle : { width: '40px', height: '40px' };
    return (
        <div className="loading-overlay" style={{ ...enhancedMaskStyle }}>
            <div className="loading-spinner me-2" style={{ ...enhancedLoadingStyle }}></div>
            <span>Loading...</span>
        </div>
    );
};
...
const createLoadingInstance = (maskStyle: any = {}, loadingStyle: any = {}): LoadingInstance => {
    // 创建容器
