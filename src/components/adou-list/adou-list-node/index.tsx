...
const ListNode = ({
    ...
    const handleLoadNode = async (clickNode: any) => {
        ...
        if (parentLoadNodeRes) {
            updatedNode.children = parentLoadNodeRes.map((item: any, index: number) => {
                let nodeInfo = {
                    ...item,
    ...
    return (
        ...
        <ListNodeWrapper $activeFontColor={activeFontColor} $activeBgc={activeBgc} className="list-node-wrapper">
            <div className="list-node-wrapper">
                {/* 每个树节点 */}
                <div className={`node-item-list  ${showLine ? 'show-line' : ''} ${node.isEachLast ? 'each-last' : ''} ${node.isFirst ? 'first' : ''}`}>
                    ...
                    >
                        ...
                        >
                            ...
                            ></i>
                            <i className={`icon fa ${deleteIconClass || 'fa-trash text-danger'}`} onClick={(e) => handleOptIconClick(e, 'delete', node)}></i>
                        </div>
                    </div>
