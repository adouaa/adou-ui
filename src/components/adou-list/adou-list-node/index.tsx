    ...
    return (
        ...
        <div className="list-node-wrapper">
            ...
            <div className="node-item">
                ...
                <div
                    className={`left-content ${Number(activeId) === Number(node.id) && 'active'}`}
                ...
                >
                    ...
                    {isTree && node.children && node.children.length > 0 && (
                        <i style={{ fontSize: '16px' }} onClick={(e) => handleFolderIconClick(node, e)} className={`icon fa fa-caret-${isExpanded ? 'down' : 'right'}`}></i>
                    )}
                    {/* 节点名字 */}
                    <span onClick={(e) => handleNodeNameClick(node, e)} className={`item-name ${node.children && node.children.length > 0 ? 'has-children' : 'no-children'}`}>
                        {node.name}
                    </span>
