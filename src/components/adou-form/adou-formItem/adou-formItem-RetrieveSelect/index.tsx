...
const MultipleSelect: React.FC<MultipleSelectProps> = (props: MultipleSelectProps) => {
    const { defaultValue, showSelected = true, options, size, className, disabled = false, setFormItemValue, onInputChange, onMultipleSelectChangeOK } = props;
    ...
    const handleDeleteItem = (item: any) => {
        ...
        // onInputChange && onInputChange(selectedList);
    ...
    const handleWrapperClick = (e: any) => {
        ...
        const position = getAbsolutePositionOfStage(multipleSelectWrapperFormControlRef.current, 0, 0);
    ...
    useEffect(() => {
        window.addEventListener('click', (e: any) => {
            ...
            let classNames = [
                'multiple-input',
                'option-icon',
                'option-item multiple-select-active',
                'select-list',
                'selected-option',
                'multiple-select-wrapper form-control ',
                'multiple-select-wrapper form-control focus',
            ];
    ...
    return (
        <>
            <div
                ...
                className={`multiple-select-wrapper form-control ${isHighlighted ? 'focus' : ''}`}
            >
                <div ref={selectListRef} className="select-list">
                    {showSelected &&
                        ...
                        selectedOptions?.map((option, index) => {
                            return (
                                <div ref={selectedOptionRefs[index]} className="selected-option" key={option.value} style={{}}>
                                    {option.label}
                                    <span onClick={() => handleDeleteItem(option)} className="option-icon">
                                        x
                                    </span>
                                </div>
                            );
                        ...
                        {optionList?.map((option: any, index: number) => {
                            ...
                            return (
                                <div
                                    ...
                                    className={`option-item ${option.selected && 'multiple-select-active'}`}
                                >
                                    {option.label}
                                </div>
                            );
