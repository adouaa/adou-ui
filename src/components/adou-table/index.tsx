import React, { Fragment, ReactElement, useCallback, useEffect, useId, useImperativeHandle, useRef, useState } from 'react';
...
const Table = (props: TableProps) => {
            ...
            data.children.map((childData: any, index: number) => (
                <Fragment key={childData[id]}>
                    <tr
                        ...
                        className={`tr-content tr-content ${childData.checked === true ? 'tr-checked' : ''} ${
                        ...
                        return (
                            ...
                            <Fragment key={data[id] + uniqId}>
                                <tr
                                    ...
                                    key={rowIndex}
                                    className={`tr-content ${data.checked === true ? 'tr-checked' : ''} ${data.highlight === true ? 'tr-highlight' : ''}`}
                                    style={{
                                        ...(clickChecked || trPointer ? { cursor: 'pointer' } : ''),
                                        ...(!tableBorderd ? { borderBottom: '1px solid #f0f0f0' } : {}),
                                    }}
    ...
    const recursiveUpdateParentChecked = (child: any, parentData: any, id: any) => {
        ...
        // 如果找到该节点的 父节点
        if (!isEmptyO(parent)) {
            const allChildrenChecked = parent.children.every((child: any) => child.checked === true);
            const someChildrenChecked = parent.children.some((child: any) => child.checked === true);
    ...
    // 点击 单/复选框
    const handleCheckboxChange = (row: any, e: any, key: 'checked' | 'highlight' = 'checked') => {
        e.stopPropagation();
        ...
        const recursiveUpdateOtherTableDataCheck = (data: any, row: any, key: 'checked' | 'highlight' = 'checked') => {
            return data.map((item: any) => {
                if (item[id] !== row[id]) {
                    item[key] = false;
                    if (item.children?.length) {
        ...
        setTimeout(() => {
            ...
            if (tableWrapperRef.current && tableBody) {
                ...
                tableWrapperRef.current.scrollTo({
                    top: tableBody.clientHeight, // 这边必须是 tableBody 的高度，不然高度不对
                    behavior: 'smooth',
                });
            }
        }, 100);
    ...
    useEffect(() => {
        ...
        if (!isDataLoaded) {
            // 必须加个延时，不然还是会渲染 暂无数据~。if 条件好像可以不需要
            setTimeout(() => {
                setIsDataLoaded(true);
            }, 300);
        }
    ...
    return (
        <>
            ...
            >
                ...
                {paginatedData.length === 0 && isDataLoaded && tableData.length === 0 && <div className="text-center text-secondary py-2">暂无数据~</div>}
                {renderPagination()}
            </div>
        </>
    );
