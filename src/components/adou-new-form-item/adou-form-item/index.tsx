...
const FormItem = ({
   ...
   */
    ...
    const enhancedChildren = React.Children.map(Array.isArray(children) ? children.filter(Boolean) : children, (child: any, index: number) => {
    ...
    return (
        <div
            className={`adou-form-item-wrapper ${suffix ? 'd-flex align-items-center' : ''} ${wrapperClassName ? wrapperClassName : generateWrapperCls()}`}
            style={{
                width: wrapperWidth,
        ...
        >
            ...
            <div
                className={`adou-form-item-content flex-fill ${judgeFormItemContentCls()} ${isError ? 'border-danger' : ''} ${
            ...
            {suffix && <div className="suffix-content ms-2">{suffix}</div>}
        </div>
    );
};
