        ...
        if (isShow) {
            ...
            if (multiple) {
                ...
                if (mode === 'common') {
                    ...
                    handleValidate(selectValue?.[valueKey] || selectValue?.[labelKey] || inputRef.current.value);
                }
            }
            setClosing(true);
            setTimeout(() => {
            ...
            if (showDefaultValue || showSearch || mode === 'liveSearch') {
                if (multiple) {
                    ...
                    if (typeof defaultValue !== 'object') {
                        ...
                        let selectOption = options?.find((option) => option?.[valueKey] === defaultValue);
                        if (!selectOption) {
                            // 没找到，则直接赋值
                            selectOption = {
        ...
        >
            ...
            <div className="adou-select-form-content input-group">
                <div
                    ...
                    className={`adou-select d-flex align-items-center px-2 ${selectContentExternalCls || ''} ${isFocus ? 'adou-form-control-focus' : ''}`}
                    style={{
                        textAlign: 'left',
                ...
                >
                    ...
                    {multiple || mode === 'tags' ? (
                      ...
                      selectValue?.[valueKey] === false ? (
                        ellipsis ? ( // 如果 ellipsis 为 true，则需要超出省略
                            ...
                            <div className={`adou-select-value  ${contentWrap ? 'ellipsis-1' : ''}`} style={{ ...(!showSearch && !filterOption ? { flex: 1 } : {}) }}>
                                {selectValue[labelKey]}
                            </div>
                        )
                    ) : (
                        // 情况五：如果 不是多选，并且 selectValue 没有值，则展示 empty-placeholder(因为没有 input)
                        mode !== 'liveSearch' && !showSearch && !filterOption && <div className="select-value-empty-placeholder flex-fill text-secondary">{placeholder}</div>
                    )}
                    {(showSearch || filterOption || mode === 'liveSearch') && !multiple && mode !== 'tags' && (
                        <div className="adou-select-input-box flex-fill" title={selectValue.title || selectValue?.[labelKey] || selectValue}>
                            <input
                                placeholder={selectValue?.[valueKey] ? '' : placeholder}
                                onFocus={handleInputFocus}
                                ref={inputRef}
                    ...
                    {clearable && isEnter && (multiple ? selectValueList.length : selectValue?.[valueKey || labelKey]) ? (
                        ...
                        <div className="adou-select-common-sufiix-content text-secondary" style={{ textAlign: 'right', whiteSpace: 'nowrap' }}>
                            {commonSuffixContent}
                        </div>
                    ) : null}
                    {commonSuffixIcon ? <i onClick={handleClickCommonSuffixIcon} className={`${commonSuffixIcon} adou-select-common-suffix-icon`}></i> : null}
                    {showIcon ? (
                        <div
                ...
                {addonAfter && (
                    <div className="addon-after input-group-text" style={{ ...addonAfterStyle }}>
                        {addonAfter}
                    </div>
                )}
            </div>
