        ...
        >
                                ...
                                selectedOptions?.map((option, index) => {
                                    return (
                                        <div
                                            ...
                                            className={`${single ? 'adou-retrieve-selectd-option-single ellipsis-1' : 'adou-retrieve-selectd-option'}`}
                                            key={option[valueKey]}
                                        >
                                            {option[labelKey]}
            ...
            {ReactDOM.createPortal(
                <div
                    ...
                    className={`adou-retrieve-select-content ${showOptions ? 'adou-retrieve-select-content-open' : ''}`}
                >
                    {!readOnly &&
                        isOpen &&
                        (optionList?.length > 0 ? (
                            optionList?.map((option: any, index: number) => {
                                return (
                                    <div
                                        ...
                                        className={`adou-retrieve-select-option ${option.selected && 'adou-retrieve-select-option-active'} ${
                                            focusedIndex === index && 'adou-retrieve-select-option-focused'
                                        }`}
                                    >
                                        {option[labelKey]}
                                    </div>
                            ...
                            <div className="adou-retrieve-select-none-match ps-2 font-italic">No content</div>
                        ))}
                </div>,
                document.body
            )}
