...
import Select from '../adou-new-form/adou-select';
...
const Calendar = ({
    ...
    const generateCurrentMonthInfo = (currentDate?: any) => {
        const { firstDate, lastDate }: any = getCurrentMonthFirstAndLastDate(currentDate);
    ...
    return (
        <div className="calendar-container" ref={calendarContainerRef} style={{ height: wrapperHeight, width: wrapperWidth }}>
                ...
                <div className="calendar-option">
                    ...
                    <div className="left">
                        ...
                        <div className="year-select ms-2 me-2">
                            <Select showLabel={false} defaultValue={currentDate.getFullYear()} onChange={handleCurrentYearChange} width={'80px'} options={years}></Select>
                        </div>
                    </div>
                    <div className="right">
                            ...
                            <tr key={weekIndex}>
                                {week.map((dayInfo: any, dayIndex: number) => (
                                    <td onDoubleClick={() => handleTdDoubleClick(dayInfo)} onClick={() => handleTdClick(dayInfo)} key={dayInfo.id} className={`calendar-cell`}>
                                        <div className={`calendar-cell-content-box`} style={{ height: cellHeight, minHeight: minCellHeight }}>
                                            <span className={`calendar-cell-content-day ${!dayInfo.isCurrentMonth ? 'gray' : ''}  ${activeId == dayInfo.id ? 'active' : ''}`}>
                                                {dayInfo.day}
                                            </span>
                                            <div className="calendar-cell-content-event">{renderEventUIFn ? renderEventUIFn(dayInfo) : dayInfo.event}</div>
                                        </div>
                                    </td>
                                ))}
                            </tr>
