...
const RetrievrSelect: React.FC<RetrieveSelectProps> = React.forwardRef((props: RetrieveSelectProps, ref) => {
    ...
    const { name, defaultValue, showSelected = true, options, size, className, disabled = false, setFormItemValue, onChangeOK, onMultipleSelectChangeOK } = props;
    ...
    return <>
        <div ref={multipleSelectWrapperFormControlRef} tabIndex={0} onBlur={handleBlur} onClick={handleWrapperClick} className={`multiple-select-wrapper form-control ${isHighlighted ? "focus" : ""}`}>
            <select multiple style={{display: "none"}} name={name}>
