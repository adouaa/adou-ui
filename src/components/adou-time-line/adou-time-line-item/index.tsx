        ...
        <div ref={timeLineItemWrapperRef} className="time-line-item-wrapper">
            <div className={`content`}>
                <div className="icon d-flex justify-content-center align-items-center">
                    <i
                        className={`fs-5 fa-solid  ${data.isFinish ? data.checkedIcon || 'fa-circle-check finished' : data.unCheckedIcon || 'fa-solid fa-circle-xmark un-finish'}`}
                    ></i>
                </div>
                ...
                {!isLasted && <div style={{ width: timeLineWidth, height: timeLineHeight }} className={`time-line-line ${data.isFinish ? 'finished' : 'un-finish'}`}></div>}{' '}
                {/* 连接线 */}
            </div>
        </div>
    );
