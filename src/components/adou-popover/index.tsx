...
const Popover: React.FC<PopoverProps> = forwardRef(
    ({ title, trigger = 'click', show = true, content, position = 'top', children, bgc, color, borderColor, wrapperClassname, onClose, onShowChange }, ref) => {
        ...
        // 点击 触发内容 的时候也不执行这个钩子
        useClickOutside([popoverRef, triggerRef], handleClosePopover, isVisible && isShow && Boolean(popoverRef.current));
        ...
        return (
            <div className={`adou-popover-wrapper ${wrapperClassname || ''}`}>
                <div ref={triggerRef} className="trigger-content" onMouseEnter={handleMouseEnterPopover} onClick={handleContentClick} onMouseLeave={handleMouseLeave}>
                    {children}
                </div>
                {content && isShow && (
                    <div
                        ...
                        className={`adou-popover p-2 ${isVisible ? 'show-popover' : ''} adou-popover-${position}`}
