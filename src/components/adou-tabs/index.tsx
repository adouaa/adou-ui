...
const Tabs = (props: TabsProps) => {
    ...
    const { children, handleLabelClick, activeIndex = 0, activeLabelColor = "#409eff", tabStyle = "common" } = props;
        ...
        return <>
            {tabStyle === "common" ?
                ...
                <ul className="nav nav-tabs m-2">
                    {tabItems.map((child: any, index: number) => {
                        return <li key={index} className="nav-item d-flex" onClick={() => handleLabelClickFn(index)}>
                            <a style={{ marginLeft: index === 0 ? "10px" : "", color: index === activeIndex ? child.props.activeLabelColor || activeLabelColor : "" }} className={`${index === activeIndex ? "active" : ""} nav-link`} aria-current="page" href="#"><i className={child.props.prefixIcon + " me-1"}></i>{child.props.label}</a>
                        </li>
                    })}
                </ul>}
        </>
