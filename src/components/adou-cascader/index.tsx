...
const Cascader = forwardRef(
    (
        ...
        const handleWrapperClick = () => {
            ...
            setIsShow(true);
            setIsOpen(true);
            const position = getAbsolutePositionOfStage(cascaderWrapperRef.current, 0, 0);
        ...
        useEffect(() => {
            if (defaultValue?.length) {
                ...
                defaultValue.forEach((item: any, index: number) => {
                    switch (index) {
                        case 0:
                            firstItem = data1.find((dataItem: any) => dataItem[valueKey] === item);
                            if (firstItem) {
                        ...
                        case 1:
                            ...
                            secondItem = firstItem?.children.find((dataItem: any) => dataItem[valueKey] === item);
                            if (secondItem) {
                        ...
                        case 2:
                            ...
                            thirdItem = secondItem?.children.find((dataItem: any) => dataItem[valueKey] === item);
                            if (thirdItem) {
        ...
        useClickOutside([cascaderWrapperRef, optionRef], handleClose, cascaderWrapperRef.current && isFocused);

        return (
            <div className={`cascader-wrapper ${isFormItem ? 'mb-3' : ''}`} style={{ ...(inline ? { marginRight: '15px' } : ''), width, ...(inline && !width ? { flex: 1 } : {}) }}>
                <div className={`left  label-in-${labelPosition}`}>
                    {String(isFocused)}111
                    {showLabel && (
                        <div className="label-box" style={{ width: labelWidth, color: labelColor }}>
                            {label}
                        </div>
                    )}
                        ...
                        >
                            <div className="left">
                                <div className={`select-data-box`}>
                                    ...
                                    <div className="third-select-data select-data">{thirdSelectData?.[labelKey]}</div>
                                </div>
                                {filterable && (
                            ...
                            <div className="right">
                                <div className="opt-box">
                                    {showClearIcon ? (
                                        <i onClick={handleClear} className="clear-icon fa-regular fa-circle-xmark"></i>
                                    ) : (
                                        <i className={`opt-icon fa-solid fa-chevron-right ${isFocused ? 'focus' : ''}`}></i>
                                    )}
                                </div>
                            </div>
                            ...
                            {ReactDOM.createPortal(
                                <div
                                    ...
                                    className={`cascader-option-box ${isOpen ? 'cascader-option-box-open' : ''}`}
                                >
                                    {isShow && (
                                        <div ref={optionRef} className={`cascader-option-content`}>
                                            {data1.length > 0 ? (
                                                <div className="data1-box">
                                                    ...
                                                    {data1.map((item: any, index: number) => (
                                                        <div
                                                            key={index}
                                                            className={`cascader-option ${firstSelectData[valueKey] === item[valueKey] ? 'cascader-option-active' : ''}`}
                                                            onClick={(e) => handleChange(e, 1, item)}
                                                            style={{
                                                                color: firstSelectData?.[valueKey] === item[valueKey] ? activeColor.font : '#000',
                                                                backgroundColor: firstSelectData?.[valueKey] === item[valueKey] ? activeColor.bgc : '',
                                                            }}
                                                        >
                                            ...
                                            {data2.length > 0 ? (
                                                <div className={`data2-box ${showData2 ? 'data2-box-open' : ''}`}>
                                                    {data2.map((item: any, index: number) => (
                                                        <div
                                                            key={index}
                                                            className={`cascader-option ${secondSelectData[valueKey] === item[valueKey] ? 'cascader-option-active' : ''}`}
                                                            onClick={(e) => handleChange(e, 2, item)}
                                                            style={{
                                                                color: secondSelectData?.[valueKey] === item[valueKey] ? activeColor.font : '#000',
                                                                backgroundColor: secondSelectData?.[valueKey] === item[valueKey] ? activeColor.bgc : '',
                                                            }}
                                                        >
                                                            <div className="text">{item[labelKey]}</div>
                                                            {item.children?.length > 0 && (
                                            ...
                                            {data3.length > 0 ? (
                                                <div className={`data3-box ${showData3 ? 'data3-box-open' : ''}`}>
                                                    {data3.map((item: any, index: number) => (
                                                        <div
                                                            key={index}
                                                            className={`cascader-option ${thirdSelectData[valueKey] === item[valueKey] ? 'cascader-option-active' : ''}`}
                                                            onClick={(e) => handleChange(e, 3, item)}
                                                            style={{
                                                                color: thirdSelectData?.[valueKey] === item[valueKey] ? activeColor.font : '#000',
                                                                backgroundColor: thirdSelectData?.[valueKey] === item[valueKey] ? activeColor.bgc : '',
                                                            }}
                                                        >
                                                            {item[labelKey]}
                                                        </div>
